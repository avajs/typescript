...
const withProvider = createProviderMacro('ava-8', '8.0.0');
...
test('negotiates ava-8 protocol', withProvider, t => t.plan(2));
...
test('main() extensions: defaults to [\'ts\', \'mts\']', withProvider, (t, provider) => {
	t.deepEqual(provider.main({config: {rewritePaths: {'src/': 'build/'}, compile: false}}).extensions, ['ts', 'mts']);
});
